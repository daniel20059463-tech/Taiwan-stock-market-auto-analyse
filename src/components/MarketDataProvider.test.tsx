...
describe("MarketDataProvider", () => {
  ...
  it("stores order book and trade tape snapshots from worker messages", async () => {
    mockDesktopBridge.isDesktopRuntime.mockReturnValue(false);
    ...
    await act(async () => {
      WorkerMock.instances[0].dispatchMessage({
        ...
        asks: [{ level: 1, price: 100.5, volume: 12 }],
        bids: [{ level: 1, price: 100.0, volume: 15 }],
      });
      WorkerMock.instances[0].dispatchMessage({
        type: "TRADE_TAPE_SNAPSHOT",
        symbol: "2330",
        timestamp: 1_700_000_000_100,
        rows: [{ time: "09:00:01", price: 100.0, volume: 3, side: "outer" }],
      });
    });

    expect(useMarketStore.getState().orderBooks.get("2330")).toEqual({
      symbol: "2330",
      timestamp: 1_700_000_000_000,
      asks: [{ level: 1, price: 100.5, volume: 12 }],
      bids: [{ level: 1, price: 100.0, volume: 15 }],
    });
    expect(useMarketStore.getState().tradeTapes.get("2330")).toEqual({
      symbol: "2330",
      timestamp: 1_700_000_000_100,
      rows: [{ time: "09:00:01", price: 100.0, volume: 3, side: "outer" }],
    });
