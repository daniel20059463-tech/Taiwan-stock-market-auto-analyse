...
let quoteDetailSymbol: string | null = null;
const preloadedHistorySymbols = new Set<string>();
...
function postOrderBookSnapshot(
  symbol: string,
  timestamp: number,
  asks: { level: number; price: number; volume: number }[],
  bids: { level: number; price: number; volume: number }[],
): void {
  const message: WorkerOutboundMessage = {
...
function postTradeTapeSnapshot(
  symbol: string,
  timestamp: number,
  rows: { time: string; price: number; volume: number; side: "outer" | "inner" | "neutral" }[],
): void {
  const message: WorkerOutboundMessage = {
...
async function handleSocketPayload(data: string | ArrayBuffer | Blob): Promise<void> {
  ...
  try {
    ...
    if (parsed && typeof parsed === "object" && parsed.type === "ORDER_BOOK_SNAPSHOT" && typeof parsed.symbol === "string") {
      const asks = Array.isArray(parsed.asks)
        ? parsed.asks
            .map((level) => {
              const bookLevel = asNumber(level.level);
              const price = asNumber(level.price);
              const volume = asNumber(level.volume);
              if (bookLevel === undefined || price === undefined || volume === undefined) {
                return null;
              }
              return { level: bookLevel, price, volume };
            })
            .filter((level): level is { level: number; price: number; volume: number } => level !== null)
        : [];
      const bids = Array.isArray(parsed.bids)
        ? parsed.bids
            ...
            .filter((level): level is { level: number; price: number; volume: number } => level !== null)
        : [];
      postOrderBookSnapshot(parsed.symbol, parsed.timestamp ?? Date.now(), asks, bids);
      return;
    }
    if (parsed && typeof parsed === "object" && parsed.type === "TRADE_TAPE_SNAPSHOT" && typeof parsed.symbol === "string") {
        ...
        ? parsed.rows
            .map((row) => {
              ...
              if (
                ...
                price === undefined ||
                volume === undefined ||
                (side !== "outer" && side !== "inner" && side !== "neutral")
              ) {
                return null;
              }
              return { time, price, volume, side };
            })
            .filter((row): row is { time: string; price: number; volume: number; side: "outer" | "inner" | "neutral" } => row !== null)
        : [];
      postTradeTapeSnapshot(parsed.symbol, parsed.timestamp ?? Date.now(), rows);
      return;
    }
...
function connectFeed(): void {
  ...
  socket.onopen = () => {
    ...
    if (quoteDetailSymbol) {
      socket?.send(JSON.stringify({ type: "subscribe_quote_detail", symbol: quoteDetailSymbol }));
    }
  };
...
self.onmessage = (event: MessageEvent<WorkerInboundMessage>) => {
  ...
  switch (message.type) {
    ...
    case "PRELOAD_HISTORY": {
      const symbols = message.symbols.filter((symbol) => !!symbol && !preloadedHistorySymbols.has(symbol));
      if (symbols.length === 0) {
        break;
      }
      for (const symbol of symbols) {
        preloadedHistorySymbols.add(symbol);
      }
      if (socket?.readyState === WebSocket.OPEN) {
        socket.send(JSON.stringify({ type: "preload_history", symbols, months: message.months ?? 6 }));
      }
      break;
    }
    case "SUBSCRIBE_QUOTE_DETAIL":
      quoteDetailSymbol = message.symbol;
      if (socket?.readyState === WebSocket.OPEN) {
        if (quoteDetailSymbol) {
          socket.send(JSON.stringify({ type: "subscribe_quote_detail", symbol: quoteDetailSymbol }));
        } else {
          socket.send(JSON.stringify({ type: "unsubscribe_quote_detail" }));
        }
      }
      break;
    default:
